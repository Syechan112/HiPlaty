...
import { Send, UserMinus, MessageCircle, Edit2, Trash2, X, Copy, Check, Users, ArrowLeft } from 'lucide-react';
...
export function ChatActiveConversation({
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 bg-white">
      {/* Header Bar */}
      <div className="p-3 sm:p-4 border-b border-slate-100 flex items-center justify-between gap-2 sm:gap-3 bg-white">
        <div className="flex items-center gap-2 sm:gap-3 truncate">
          {onBack && (
            <button
              type="button"
              onClick={onBack}
              className="p-2 -ml-1 text-slate-500 hover:text-slate-900 hover:bg-slate-100 rounded-xl transition-colors cursor-pointer md:hidden"
              title="Kembali ke Daftar Teman"
            >
              <ArrowLeft className="w-5 h-5" />
            </button>
          )}
          <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-slate-900 text-white font-bold text-xs flex items-center justify-center shrink-0">
            {(activeFriend.name || 'U').charAt(0).toUpperCase()}
          </div>
          <div className="truncate">
            <div className="flex items-center gap-2">
              <h2 className="font-bold text-slate-900 text-xs sm:text-sm truncate">{activeFriend.name || 'Teman'}</h2>
